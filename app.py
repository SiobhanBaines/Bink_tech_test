...
def main():
    ...
    print("\nThis is the first 5 tenants from the \
            csv file after loading into a list:\n")
    print(mast_data_list[:5])

    input("\nPress Enter to see a list of columns to sort the data by.")

    sorted_list = sort_data(mast_data_list, headings)
    print("\nThese are the first 5 tenants from the sorted list:\n")
    print(sorted_list[:5])

    lease_list = leased_years(mast_data_list)
    print("\nThis is a list of all the tenants for \
            the above number of lease years\n")
    print(lease_list)

    total_rent = calc_rent(lease_list)
    print(f"\nThe total rent for these tenants is: £{total_rent:.2f}")
